...
-- ============================================================
-- 1. rpc_get_ai_farm_context
-- Dok 5 §6.1 / §6.5: get_farm_context → rpc.get_ai_farm_context
-- Returns full farm snapshot for AI context loading (§5.1 farm_context_snapshot)
-- ============================================================
    ...
    return (
        select jsonb_build_object(
            ...
            'active_feeding_plan', (
                ...
                where  fp.farm_id = f.id and fp.status = 'active'
...
comment on function public.rpc_get_ai_farm_context(uuid, uuid) is
    'Dok 5 §5.1 / §6.1: Returns full farm context snapshot for AI Gateway.
     Includes herd groups, active feeding plan, upcoming vaccinations, tasks, membership.
     Called via supabase.rpc("rpc_get_ai_farm_context") from Python AI Gateway.
     SECURITY DEFINER: service_role JWT compatible. p_organization_id for ownership.';

-- ============================================================
-- 2. rpc_upsert_herd_group
-- Dok 5 §6.1: update_herd_group / create_herd_group → rpc.upsert_herd_group
-- Creates or updates a herd group. data_source='ai_extracted', confidence=50.
-- Inserts HerdEvent for count/weight changes.
-- ============================================================
create or replace function public.rpc_upsert_herd_group(
    p_organization_id       uuid,
    p_farm_id               uuid,
    p_animal_category_code  text,           -- English code: 'BULL_CALF', 'COW', etc.
    p_head_count            int,
    p_avg_weight_kg         numeric         default null,
    p_breed_id              uuid            default null,
    p_herd_group_id         uuid            default null,  -- if set: UPDATE existing group
    p_actor_id              uuid            default null,  -- AI Gateway passes conversation user_id
    p_ai_context            jsonb           default null   -- D103 convention
)
returns jsonb
language plpgsql
security definer
...
begin
    ...
    if p_herd_group_id is not null then
        -- UPDATE path
        select head_count, avg_weight_kg into v_old_count, v_old_weight
        ...
        -- INSERT path: upsert by (farm_id, animal_category_id)
...
comment on function public.rpc_upsert_herd_group(uuid,uuid,text,int,numeric,uuid,uuid,uuid,jsonb) is
    'Dok 5 §6.1: Creates or updates a herd group from AI extraction (data_source=ai_extracted, confidence=50).
     p_animal_category_code: English DB codes (BULL_CALF, STEER, HEIFER_YOUNG, COW, etc.).
     p_herd_group_id=null: INSERT with ON CONFLICT upsert by (farm_id, category).
     p_herd_group_id set: UPDATE existing group (ownership validated).
     Publishes farm.herd_group.updated to platform_events.';
