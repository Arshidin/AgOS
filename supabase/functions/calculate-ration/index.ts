 ...
 * Input: animal params + farm_id. Output: RationVersion via rpc_save_ration.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.organization_id || !body.farm_id || !body.animal_category_id) {
    ...
    // 1. Load feed inventory with nutrient data
    ...
    const feeds = inventory.map((inv: any) => {
      ...
      return {
        ...
        is_validated: fi.is_validated, is_roughage: roughageCodes.has(fi.code),
      };
    });

    // 4. Greedy allocation: roughage first (50% DM), then concentrates
    ...
    for (const f of feeds.filter((f: any) => f.is_roughage).sort((a: any, b: any) =>
      ...
      const maxDm = Math.min((f.available_kg / body.head_count) * dm, roughageTarget - roughageAlloc, remaining);
      if (maxDm <= 0) continue;
      const kg = maxDm / dm;
      items.push({
    ...
    for (const f of feeds.filter((f: any) => !f.is_roughage).sort((a: any, b: any) =>
      ...
      const dm = (f.nc.dm_pct || 88) / 100;
      const maxDm = Math.min((f.available_kg / body.head_count) * dm, remaining);
      if (maxDm <= 0) continue;
      const kg = maxDm / dm;
      items.push({
        feed_item_id: f.id, feed_item_code: f.code,
    ...
    const unvalidated = feeds.filter((f: any) => !f.is_validated && items.some((i: any) => i.feed_item_id === f.id)).map((f: any) => f.id);
    const status = deficiencies.length === 0 ? "optimal" : items.length > 0 ? "feasible" : "infeasible";

    const results = {
      total_cost_per_day: round2(totCost), total_cost_per_month: round2(totCost * 30),
      total_dm_kg: round2(totDm), nutrients_met: met,
      nutrient_values: { dm_kg: round2(totDm), me_mj: round2(totMe), cp_g: round2(totCp), ndf_pct_dm: round1(roughPct), ca_g: round2(totCa), p_g: round2(totP) },
      nutrient_requirements: { dm_kg: round2(scaled.dm_kg), me_mj: round2(scaled.me_mj), cp_g: round2(scaled.cp_g), ndf_pct_dm_min: scaled.ndf_pct_min, ca_g: round2(scaled.ca_g), p_g: round2(scaled.p_g) },
      roughage_pct_dm: round1(roughPct), deficiencies,
      warnings: unvalidated.length > 0 ? ["Q37: корма без валидации зоотехника"] : [],
      solver_status: status, feed_items_unvalidated: unvalidated,
    };

    // 6. Save via rpc_save_ration
    const { data: saveResult, error: saveError } = await supabase.rpc("rpc_save_ration", {
      ...
      p_breed_id: body.breed_id || null, p_period_type_id: body.period_type_id || null,
      p_avg_weight_kg: body.avg_weight_kg, p_head_count: body.head_count || 1,
      p_objective: body.objective || "growth", p_shelter_type: body.shelter_type || "combined",
      p_target_daily_gain_kg: body.target_daily_gain_kg || null,
      p_ration_id: body.ration_id || null,
      p_items: items, p_results: results, p_calculated_by: "edge_function",
    });

    if (saveError) return jsonResponse({ error: saveError.message }, 500);
    return jsonResponse({ ...saveResult, items, results });
  } catch (err) {
