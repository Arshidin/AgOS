...
-- ============================================================
-- PATCH 5: Вспомогательная функция — превью каскада
-- "Что сдвинется если я перенесу эту фазу?"
-- Эксперт может посмотреть ДО применения изменения.
-- ============================================================
...
returns table (
    ...
    shift_days      int,
    date_type       text,
    depth           int   -- уровень вложенности каскада
)
language sql
security definer
set search_path = public, pg_temp   -- D-2 fix
as $$
    -- Рекурсивный CTE для обхода дерева зависимостей
    with recursive cascade_tree as (
        -- База: сдвигаемая фаза
        select
            fp.id                       as phase_id,
            fp.name_ru                  as phase_name,
            fp.start_date               as current_start,
            fp.end_date                 as current_end,
        ...
        -- Рекурсия: зависимые sequential фазы
        select
            dep.id,
            dep.name_ru,
            dep.start_date,
            dep.end_date,
            parent.new_end + dep.lag_days,
            parent.new_end + dep.lag_days + (dep.end_date - dep.start_date),
            (parent.new_end + dep.lag_days) - dep.start_date,
            dep.date_type,
            parent.depth + 1
        from public.farm_phases dep
        join cascade_tree parent on dep.depends_on_phase_id = parent.phase_id
        where dep.date_type = 'sequential'
          and dep.status not in ('completed', 'skipped')
    )
    select * from cascade_tree
    order by depth, new_start;
$$;

comment on function public.fn_preview_cascade(uuid, date) is
    'D104: Превью каскада без изменения данных.
     Эксперт видит что именно сдвинется ДО нажатия "Применить".
     Expert Console: показать таблицу перед подтверждением.
     AI Gateway: может использовать для объяснения фермеру:
       "Если перенести случку на 21 день, диагностика стельности
        сдвинется с 15 мая на 5 июня, а сухостой — с октября на ноябрь."';

-- ============================================================\n-- MIGRATION COMPLETE\n-- ============================================================\n-- Schema changes (zero breaking):\n--   phase_templates: +3 columns (date_type, depends_on_phase_code, lag_days_after_dependency)\n--   farm_phases:     +4 columns (depends_on_phase_id, lag_days, date_type, original_duration_days)\n--\n-- New RPCs:\n--   fn_shift_phase_cascade(phase_id, new_start_date, actor_id) → jsonb\n--   fn_preview_cascade(phase_id, new_start_date) → table\n--\n-- Seed updates:\n--   phase_templates: date_type + depends_on_phase_code установлены для cow_calf\n--\n-- D104 CLOSED: Механизм каскадного сдвига дат реализован.\n--\n-- Audit fixes applied in this version:\n--   D-2  ✅  fn_shift_phase_cascade + fn_preview_cascade: SECURITY DEFINER\n--            SET search_path = public, pg_temp\n--   D-5  ✅  CALVING добавлена в список CALENDAR фаз в header comment\n--   L-5  ✅  Исправлен вводящий в заблуждение комментарий про транзакцию.\n--            Добавлено объяснение: функция атомарна через PL/pgSQL implicit\n--            savepoint semantics. Явный BEGIN/COMMIT НЕ нужен.\n--\n-- Что намеренно НЕ автоматизировано:\n--   Автоматический каскад при изменении actual completion date фазы —\n--   это Dok 6 (UX). Зоотехник решает: \"фаза завершилась позже,\n--   каскадировать зависимые?\" Система предлагает через fn_preview_cascade,\n--   зоотехник подтверждает через fn_shift_phase_cascade.\n--   Полный автомат опасен: фаза может завершиться раньше не потому что\n--   процесс ускорился, а потому что данные введены неточно.\n-- ============================================================


-- === FROM 010: fn_generate_production_plan (initial) ===
-- ============================================================
-- AGOS Migration 010: fn_generate_production_plan
-- Project: TURAN Agricultural Operating System
-- Version: 1.0 | Date: 5 March 2026
--
-- Decision: D74 — Template → Plan
-- Closes: L-1 (Audit Report, March 2026)
--
-- Problem:
--   rpc_start_production_plan (Dok 3, RPC-33) ссылается на
--   fn_generate_production_plan() в 7+ местах (008 comment,
--   006 comment, Dok 1 §8 D103). Функция нигде не реализована.
--   Нажать "Запустить план" → RuntimeError "function does not exist".
--
-- Solution:
--   Реализуем fn_generate_production_plan() как PostgreSQL функцию.
--   Один вызов = FarmProductionPlan + все FarmPhase + FarmTask + FarmKPI.
--
-- Algorithm (из Dok 3 комментария к 008_patch_cascade.sql):
--
--   1. Валидация: ферма существует, шаблон существует и активен.
--   2. D82 check: нет активного плана на ферму (partial unique index).
--   3. INSERT FarmProductionPlan (status='draft').
--   4. Загрузить все phase_templates для cycle_template, ORDER BY sort_order.
--   5. Для каждой phase_template:
--      a. Вычислить start_date / end_date:
--         - calendar/parallel: start = cycle_start_date + offset_from_cycle_start_days
--         - sequential:        start = predecessor_end + lag_days_after_dependency
--      b. D101: найти herd_group_id по animal_category_codes
--      c. D104: резолвить depends_on_phase_code → depends_on_phase_id
--      d. INSERT FarmPhase, сохранить id в локальный map code→id
--   6. Для каждой FarmPhase: INSERT FarmTask из task_templates фазы.
--   7. Для cycle_template: INSERT FarmKPI из kpi_templates.
--   8. INSERT platform_event (ops.production_plan.started).
--   9. Вернуть plan_id.
--
--   Фазы обрабатываются в sort_order. Инвариант:
--   predecessor всегда имеет меньший sort_order, чем зависимая фаза
--   (гарантировано данными 007_ctk_seed.sql).
--   Нарушение инварианта → RAISE EXCEPTION с понятным сообщением.
--
-- Depends on: 005_ops_edu.sql, 006_patch_ops.sql, 007_ctk_seed.sql,
--             008_patch_cascade.sql, 009_patch_ai.sql
-- Called by: rpc_start_production_plan (Dok 3, RPC-33)
-- ============================================================

-- ============================================================
-- MAIN FUNCTION: fn_generate_production_plan
--
-- Параметры:
--   p_farm_id          — Ферма (farms.id)
--   p_template_id      — Шаблон ЦТК (production_cycle_templates.id)
--   p_plan_start_date  — Дата начала плана (cycle_start_date)
--                        Семантика: первый день массовых отёлов (D78)
--   p_actor_id         — user_id инициатора (для audit)
--   p_target_heads     — Расчётное поголовье (для KPI targets), NULL = из HerdGroup
--   p_herd_group_ids   — Явная привязка HerdGroup (переопределяет D101 auto-match)
--                        NULL = использовать auto-match по animal_category_codes
--   p_expert_profile_id — Зоотехник ТУРАН ведущий ферму (D79), NULL = без назначения
--   p_plan_name        — Название плана, NULL = авто: "ЦТК {year} — {farm_name}"
--
-- Возвращает: uuid (plan_id)
-- ============================================================
...
declare
    ...
    -- Map: phase_template.code → {phase_id, end_date}
    -- Используем временную таблицу (доступна в рамках транзакции)
    -- Ключ: phase_template.code | Значение: phase_id || ':' || end_date::text
...
begin
    ...
    -- Шаблон ЦТК существует и активен
    select  pct.id,
            pct.code,
            pct.name_ru,
            pct.cycle_duration_days,
            pct.is_recurring
    into    v_template
    from    public.production_cycle_templates pct
    where   pct.id        = p_template_id
      and   pct.is_active = true;

    if not found then
        raise exception 'TEMPLATE_NOT_FOUND: шаблон ЦТК % не найден или неактивен', p_template_id
            using errcode = 'P0001';
    end if;

    v_cycle_days := v_template.cycle_duration_days;

    -- D82: не больше одного активного плана на ферму
    -- Partial unique index idx_farm_plan_one_active покрывает это для status='active'.
    -- Дополнительная проверка на 'draft' во избежание мусора:
    if exists (
        select 1 from public.farm_production_plans
        where  farm_id = p_farm_id
          and  status  in ('draft', 'active')
    ) then
        raise exception 'PLAN_ALREADY_EXISTS: у фермы % уже есть активный или черновой план. '
                        'Завершите или отмените текущий план перед созданием нового.', p_farm_id
            using errcode = 'P0001';
    end if;

    -- Дата начала плана не может быть в прошлом > 90 дней
    -- (защита от случайных ошибок ввода; эксперт может переопределить)
    if p_plan_start_date < current_date - interval '90 days' then
        raise exception 'INVALID_START_DATE: дата начала % слишком далеко в прошлом (>90 дней). '
                        'Проверьте дату или обратитесь к администратору.', p_plan_start_date
            using errcode = 'P0001';
    end if;

    -- ──────────────────────────────────────────────────────
    -- 2. ВРЕМЕННАЯ ТАБЛИЦА: MAP phase_code → {phase_id, end_date}
    --    Нужна для резолвинга depends_on_phase_code → depends_on_phase_id
    --    при обходе фаз в sort_order.
    -- ──────────────────────────────────────────────────────
    ...
    -- Очистить от предыдущих вызовов в той же транзакции (edge case)
    delete from _phase_code_map;

    -- ──────────────────────────────────────────────────────
    -- 3. СОЗДАТЬ FarmProductionPlan (status='draft')
    -- ──────────────────────────────────────────────────────

    -- Вычислить дату окончания цикла
    ...
    -- ──────────────────────────────────────────────────────
    -- 4. СОЗДАТЬ FarmPhase для каждого PhaseTemplate
    --    Порядок: sort_order ASC (инвариант: предшественник всегда раньше)
    -- ──────────────────────────────────────────────────────
    ...
    for v_phase_template in
        select  pt.id                           as template_id,
                ...
                pt.name_ru,
                pt.sort_order,
                pt.offset_from_cycle_start_days,
                pt.duration_days,
                pt.animal_category_codes,
                pt.is_sale_phase,
                pt.suggested_period_type_code,
                pt.date_type,
                pt.depends_on_phase_code,
                pt.lag_days_after_dependency
    ...
    loop
        ...
        if v_phase_template.date_type = 'sequential' then
            -- Sequential: start = predecessor_end_date + lag_days
            -- Предшественник должен уже быть в _phase_code_map

            if v_phase_template.depends_on_phase_code is null then
                raise exception
                    'TEMPLATE_ERROR: фаза % (code=%) имеет date_type=sequential '
                    'но depends_on_phase_code=NULL. Исправьте шаблон ЦТК.',
                    v_phase_template.name_ru, v_phase_template.code
                    using errcode = 'P0001';
            end if;

            -- Найти предшественника в map
            select  m.end_date
            into    v_phase_start  -- временно: end_date предшественника
            from    _phase_code_map m
            where   m.phase_code = v_phase_template.depends_on_phase_code;

            if not found then
                raise exception
                    'TEMPLATE_ORDER_ERROR: фаза % (code=%) зависит от % '
                    'который ещё не создан. Проверьте sort_order в шаблоне ЦТК: '
                    'предшественник должен иметь меньший sort_order.',
                    v_phase_template.name_ru,
                    v_phase_template.code,
                    v_phase_template.depends_on_phase_code
                    using errcode = 'P0001';
            end if;

            -- v_phase_start сейчас = end_date предшественника (из select выше)
        ...
        if p_herd_group_ids is not null and array_length(p_herd_group_ids, 1) > 0 then
            ...
            where   hg.farm_id   = p_farm_id
              ...
              and   ac.code      = any(v_phase_template.animal_category_codes)
            limit 1;
            -- NULL если HerdGroup не найдена — нормальная ситуация:
            -- эксперт назначит herd_group_id вручную через Expert Console.
        end if;

        -- ── 4c. Резолвить depends_on_phase_id (D104) ──────
        ...
        if v_phase_template.depends_on_phase_code is not null then
            ...
            from    _phase_code_map m
            where   m.phase_code = v_phase_template.depends_on_phase_code;
            -- NULL если не найден — не ошибка (sequential уже вызвал exception выше)
        end if;

        -- ── 4d. Вычислить target_sale_month (D81) ─────────
        -- Если фаза = продажа: target_sale_month = первый день месяца окончания фазы
        ...
        values (
            v_plan_id,
            v_organization_id,
            v_phase_template.template_id,
            v_herd_group_id,
            v_phase_template.name_ru,
            v_phase_start,
            v_phase_end,
            v_phase_template.is_sale_phase,
            -- D81: target_sale_month = первый день месяца v_phase_end (для sale фаз)
            case when v_phase_template.is_sale_phase
                 then date_trunc('month', v_phase_end)::date
                 else null
            end,
            v_phase_template.date_type,
            v_dep_phase_id,
            v_phase_template.lag_days_after_dependency,
            'upcoming'
        )
        ...
        -- ── 4f. Сохранить в map для резолвинга зависимостей ──
        ...
        -- ── 4g. Создать FarmTask для каждого TaskTemplate фазы ──

        for v_task_template in
            select  tt.id                           as template_id,
                    tt.name_ru,
                    tt.category,
                    tt.offset_from_phase_start_days,  -- ✅ реальное имя колонки
                    tt.erp_sync_required,             -- ✅ реальное имя колонки
                    tt.creates_herd_event
            from    public.task_templates tt
            where   tt.phase_template_id = v_phase_template.template_id
            order   by tt.sort_order asc
        loop
            -- Дата задачи = start фазы + offset_from_phase_start_days задачи
            v_task_due_date := v_phase_start
                + coalesce(v_task_template.offset_from_phase_start_days, 0);
            ...
            insert into public.farm_tasks (
                ...
                due_date,
                status,
                erp_sync            -- farm_tasks.erp_sync ✅ (005_ops_edu.sql)
            )
            values (
                v_phase_id,
                v_organization_id,
                v_task_template.template_id,
                v_task_template.name_ru,
                v_task_template.category,
                v_task_due_date,
                'scheduled',
                coalesce(v_task_template.erp_sync_required, false)
            );
    ...
    -- ──────────────────────────────────────────────────────
    -- 5. СОЗДАТЬ FarmKPI из kpi_templates цикла
    -- ──────────────────────────────────────────────────────

    -- Определить расчётное поголовье для KPI
    -- Приоритет: явный p_target_heads → сумма HerdGroup фермы → 1 (default)
    if p_target_heads is not null then
        v_kpi_target_heads := p_target_heads;
    else
        select  coalesce(sum(hg.head_count), 1)
        into    v_kpi_target_heads
        from    public.herd_groups hg
        where   hg.farm_id   = p_farm_id
          and   hg.is_active = true;
    end if;

    for v_kpi_template in
        select  kt.id               as template_id,
                kt.phase_template_id,
                kt.unit,
                kt.target_value
                -- kt.metric_code НЕ существует → поле называется kt.code + kt.kpi_type
                -- kt.name_ru, kt.description_ru, kt.tolerance_pct, kt.higher_is_better
                -- существуют в kpi_templates но не нужны в farm_kpis INSERT:
                -- читаются через kpi_template_id JOIN при отображении
        from    public.kpi_templates kt
        join    public.phase_templates pt on pt.id = kt.phase_template_id
        where   pt.cycle_template_id = p_template_id
    loop
        -- Найти farm_phase_id через join (по phase_template_id)
        -- kpi_template привязан к phase_template → ищем соответствующую FarmPhase
        insert into public.farm_kpis (
            farm_phase_id,
            organization_id,
            kpi_template_id,
            target_value,    -- ✅ поле существует в farm_kpis
            unit,            -- ✅ поле существует в farm_kpis
            status
            -- plan_id, name_ru, metric_code, tolerance_pct, higher_is_better
            -- НЕ существуют в farm_kpis — читаются через kpi_template_id JOIN
        )
        select  fp.id,
                v_organization_id,
                v_kpi_template.template_id,
                v_kpi_template.target_value,
                v_kpi_template.unit,
                'pending'
        from    public.farm_phases fp
        where   fp.plan_id           = v_plan_id
          and   fp.phase_template_id = v_kpi_template.phase_template_id
        limit 1;

        if found then
            v_kpis_created := v_kpis_created + 1;
        end if;
    end loop;
    -- END kpi loop

    -- ──────────────────────────────────────────────────────
    -- 6. ОПУБЛИКОВАТЬ EVENT в platform_events (D85, Event #19)
    --    ops.production_plan.started
    -- ──────────────────────────────────────────────────────
    ...
    insert into public.platform_events (
        ...
        entity_id,
        organization_id,
        actor_type,    -- ✅ реальная колонка (001_kernel.sql)
        actor_id,      -- ✅ реальная колонка (001_kernel.sql)
        payload
        -- 'domain' и 'triggered_by' НЕ существуют в platform_events
    )
    values (
        'ops.production_plan.started',
        'farm_production_plans',   -- entity_type = имя таблицы (конвенция)
        v_plan_id,
        v_organization_id,
        'system',      -- actor_type: план генерируется системой по запросу
        p_actor_id,    -- actor_id: user инициировавший запрос
        jsonb_build_object(
            'plan_id',          v_plan_id,
            'farm_id',          p_farm_id,
...
comment on function public.fn_generate_production_plan(
    uuid, uuid, date, uuid, int, uuid[], uuid, text
) is
    'D74/L-1: Генерирует FarmProductionPlan из шаблона ЦТК.
     Один вызов создаёт: план + все фазы + все задачи + все KPI.
     Закрывает дефект L-1 (Audit Report, март 2026).
     Вызывается из rpc_start_production_plan (Dok 3, RPC-33).
     
     Алгоритм дат фаз:
       calendar/parallel → cycle_start_date + offset_from_cycle_start_days
       sequential        → predecessor_end_date + lag_days_after_dependency
     
     D101: auto-match HerdGroup по animal_category_codes.
           Если не найдено → herd_group_id=NULL (эксперт назначает вручную).
     D104: depends_on_phase_code резолвится в depends_on_phase_id.
           Копирует date_type + lag_days из шаблона.
     D82:  Бросает PLAN_ALREADY_EXISTS если у фермы уже есть draft/active план.
     D85:  Публикует ops.production_plan.started в platform_events.
     
     Возвращает plan_id (uuid). Status = "draft" — эксперт активирует вручную.
     Активация: UPDATE SET status=''active'', activated_at=now()
                 + публикация ops.plan.activated (Dok 3, RPC-33 post-step).';

-- ============================================================
-- WRAPPER RPC: rpc_start_production_plan (Dok 3, RPC-33)
--
-- Прямой вызов от Frontend (Lovable) и AI Gateway через supabase.rpc().
-- Добавляет валидацию actor = текущий пользователь (RLS bypass через
-- SECURITY DEFINER, но actor проверяется явно).
-- Активирует план сразу после генерации (для AI-инициированных запусков).
-- ============================================================
...
create or replace function public.rpc_start_production_plan(
    ...
    p_expert_profile_id uuid    default null,
    p_plan_name         text    default null,
    p_auto_activate     boolean default false,   -- true для AI Gateway (сразу активировать)
    p_ai_context        jsonb   default null     -- D103: p_ai_context convention (Dok 3 §10.5)
)
returns jsonb
language plpgsql
security definer
...
declare
    ...
    v_plan_id   uuid;
begin
    -- actor = текущий аутентифицированный пользователь
    v_actor_id := public.fn_current_user_id();

    if v_actor_id is null then
        raise exception 'UNAUTHORIZED: необходима аутентификация'
            using errcode = 'P0001';
    end if;

    -- Проверка что актор принадлежит к организации фермы
    if not exists (
        select 1
        from   public.farms               f
        ...
        raise exception 'FORBIDDEN: у пользователя нет доступа к ферме %', p_farm_id
            using errcode = 'P0001';
    end if;
    ...
    -- p_auto_activate=true: сразу переводим в 'active' (для AI-инициированных запусков)
    -- false (default): план остаётся 'draft' — эксперт проверяет и активирует вручную
    if p_auto_activate then
        ...
        -- Дополнительный event: ops.plan.activated
        insert into public.platform_events (
            event_type, entity_type, entity_id,
    ...
    -- Вернуть результат для Frontend / AI Gateway
...
exception
    when others then
        -- Стандартизированный формат ошибки для Frontend
        return jsonb_build_object(
            'error',    true,
            'code',     sqlstate,
            'message',  sqlerrm
...
comment on function public.rpc_start_production_plan(
    uuid, uuid, date, int, uuid[], uuid, text, boolean, jsonb
) is
    'Dok 3 RPC-33: Запуск производственного плана по шаблону ЦТК.
     Тонкая обёртка над fn_generate_production_plan() с:
       - auth check (actor = текущий пользователь)
       - access check (актор принадлежит к org фермы)
       - p_auto_activate: true → plan.status=''active'' + ops.plan.activated event
       - p_ai_context: convention Dok 3 §10.5 (сохраняется в platform_events.payload)
     Возвращает jsonb {plan_id, status, message}.
     При ошибке возвращает {error:true, code, message} — не бросает исключение.
     Frontend: проверяет result.error перед использованием result.plan_id.';
