...
function useInView(threshold = 0.25) {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([e]: IntersectionObserverEntry[]) => { if (e?.isIntersecting) { setVisible(true); obs.unobserve(el); } },
      { threshold },
    );
    obs.observe(el);
...
function Pill({ children, variant = "accent" }: { children: ReactNode; variant?: "accent" | "muted" | "green" }) {
  ...
  return (
    <span
      className="inline-block rounded-md px-1.5 py-0.5 text-[12px] md:px-2.5 md:py-1 md:text-[14px]"
...
function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <span className="text-[12px] md:text-[13px] font-normal" style={{ color: C.dim }}>
      {children}
    </span>
  );
}
...
function Hint({ children }: { children: ReactNode }) {
  return (
    <span className="text-[12px] md:text-[13px] font-medium mt-1.5 md:mt-2 inline-block" style={{ color: "rgba(43,24,10,0.25)" }}>
      {children}
    </span>
  );
}
